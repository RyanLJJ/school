...
def newgame(buildings, buildingAmount): #Resets values for new game
...
def Game(buildings, buildingAmount, turns): #Function for game
    ...
    while True:
        ...
        try:
            for i in range(2):
                newBuildings.append(buildings[random.randint(0, len(buildings) - 1)]) #Appends buildings to list
            ...
            if choice == 0:
                ...
                try:
                    try: #Checks for existing position
                        build = input('Build where? ')
                        col = columns.find(build[0].lower())
                        row = int(build[1]) - 1
                    ...
                    if turns == 1: #Turn 1 building can be placed anywhere
                        validPos = True
                    elif turns > 1: #Buildings after turn 1 follow adjacent rule
                        validPos = False
                    ...
                    assert validPos #Exception handling for valid position

                    turns+=1
                    map[row][col] = newBuildings[choice - 1]
                    buildings.remove(newBuildings[choice - 1])
                    buildingAmount[newBuildings[choice - 1]] = buildingAmount[newBuildings[choice - 1]] - 1
...
def scoreCalculation(turns): #Calculates the score for each building
    ...
    if turns > 16:
        ...
        def highscores():
            ...
            with open('scores.txt', 'a') as s1:
                for i in scoreList:
                    s1.write('{} '.format(i))

            with open('highscore.txt', 'a') as h1:
            ...
            with open('highscore.txt', 'r') as h2:
                for i in range(len(oldScores)):
                    highscores = h2.readline().strip('\n')
...
def mainMenu(buildings, buildingAmount, turns): #Displays main menu
    ...
    try:
        ...
        assert -1 < startOption < 4
    except:
        print('Invalid option, try again.')
        mainMenu(buildings, buildingAmount, turns)

    if startOption == 1: #Resets values of variables
        buildings, buildingAmount, turns = newgame(buildings, buildingAmount)
...
#Monument building + Building type selection
